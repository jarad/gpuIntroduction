...
int main(){
  ...
  *b = 0.0;
  ...
  cudaMalloc((void**) &ipiv, p * p * sizeof(*ipiv));
  cudaMalloc((void**) &dX, n * p * sizeof(*X));
  cudaMalloc((void**) &dXtX, p * p * sizeof(*X));
  cudaMalloc((void**) &dXtY, p * sizeof(*X));
  cudaMalloc((void**) &dbeta, p * sizeof(*X));
  cudaMalloc((void**) &dY, n * sizeof(*X));
  ...
  cudaMemcpy(dX, X, n * p * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(dY, Y, n * sizeof(float), cudaMemcpyHostToDevice);

  cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, p, p, n, 
    a, dX, n, dX, n, b, dXtX, p);

  cudaMemcpy(XtX, dXtX, p * p * sizeof(float), cudaMemcpyDeviceToHost);
  ...
  cudaMemcpy(beta, dbeta, p * sizeof(float), cudaMemcpyDeviceToHost);
