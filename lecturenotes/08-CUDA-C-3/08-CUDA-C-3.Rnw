...
% JBN: It would really be nice to have exactly the same code you showed before, but with the atomic add, showing that it works.
...
% JBN: Point to the documentation for these functions, i.e. where do you find the documentation.
...
\Huge
\begin{center}
{\bf EXAMPLE: THE ATOMIC DOT PRODUCT}
\end{center}  \LARGE

% JBN: make clear this is contrast to performing the final summation on the CPU
...
% JBN: so atomics are new to compute capability 2.0? 

NOTE: If you're using any of the above functions in your code, compile with the flag, {\tt -arch sm\_20} as above. \newline
...
\LARGE
\begin{center}
{\bf ANOTHER EXAMPLE: COUNTING THE NUMBER OF BLOCKS}
\end{center} 
...
% JBN: by stall do you mean stop running or run really slowly?
