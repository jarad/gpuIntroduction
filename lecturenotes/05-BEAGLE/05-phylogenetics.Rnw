...
\item The relative degree of genetic similarity between and among the different organisms.
\end{enumerate}
...
\begin{itemize}
\item The leaf nodes are the known, contemporary species that we care about
\item The inner nodes are hidden (possibly unknown) ancestors 
\item An edge identifies a pair of nodes in which one member evolved directly from the other.
\end{itemize}
