...
\Huge
{\bf Pointer}: An address in memory of a stored C object. \newline \newline


\begin{tabular}{|c|c|}
\hline
Address (in bytes) & Stored Value  \\ \hline 
24399440  & 3   \\ \hline
24399444  & 5 \\ \hline
24399448  & 28 \\ \hline
24399452  & 7 \\ \hline
...
\setkeys{Gin}{width=1.1\textwidth} \includegraphics[scale=0.25,angle=0]{1.png} \newpage
...
\setkeys{Gin}{width=1.1\textwidth} \includegraphics[scale=0.25,angle=0]{2.png}
\newpage
...
\setkeys{Gin}{width=1.1\textwidth} \includegraphics[scale=0.25,angle=0]{3.png} \newline
...
{\tt a} was passed to {\tt fun()} by value. \newpage
...
{\tt a} was passed to {\tt fun()} by reference. \newpage
...
The value pointed to by {\tt pa} was passed to {\tt fun()} by reference, while {\tt a} was left alone. \newpage
...
Since {\tt pa} points to {\tt a}, {\tt *pa} and {\tt a} are the same object. {\tt a} was passed to {\tt fun()} by reference. \newpage
...
Without dynamic allocation, assign {\tt *a} by copying from named variables instead of using literals. \newline

caution1.c:
\lstinputlisting[language = C]{caution1.c} $\quad$ \newline
output:
\begin{lstlisting}[language = bash]
...
The value of {\tt a} is a memory address that does not exist! 
\newpage
