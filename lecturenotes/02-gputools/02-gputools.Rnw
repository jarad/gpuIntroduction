...
% JBN: installation will fail on non-GPU systems since the compiler doesn't exist
\item Documentation:
\begin{itemize}
...
The R script, {\tt perf\_gpuQr.r}, compares the performance of {\tt gpuQr(arg)} and {\tt qr(arg)} for square matrices {\tt arg} of varying sizes. See the results on the next few slides. {\color{red} NOTE: ALL TIMES ARE IN SECONDS.}
