...
\Huge
{\color{red} Note: although convenient, the standard interface is slower than the device interface for multiple kernels because every single CULA function writes back and forth between the CPU and the GPU.}
\newpage
...
\setkeys{Gin}{width=.9\textwidth} \includegraphics[scale=0.25,angle=0]{ae_s} \newpage
