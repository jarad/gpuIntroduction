...
\end{itemize}
\end{itemize} $\quad$ \newline
There's also a ``convenience header" called ``cula.h". \newline
...
\begin{enumerate}[1. ]
\item Include the header file in your source: ``cula\_lapack.h" for the standard interface, ``cula\_lapack\_device.h" for the device interface, or ``cula.h". \newline
\item Enter in the command line:
\begin{verbatim}
nvcc -I /usr/local/cula/include -L /usr/local/cula/lib64 -lcula_core -lcula_lapack -lcublas -lcudart 
