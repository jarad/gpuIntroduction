...
\Huge
\begin{center}
{\bf REVIEW OF PARALLELISM}
\end{center} $\quad$ \newline 

{\bf Parallelism}: Running different calculations simultaneously.  \newline
...
\Huge
\begin{center}
{\bf PARALLELISM AND THE SIMD PARADIGM}
\end{center} $\quad$ \newline 
...
\begin{enumerate}[a. ]
\item The GPU makes $N \cdot M$ {\color{blue} copies} of the kernel's code, and then runs all those copies simultaneously. Those parallel copies are called {\bf threads}.  \newline
\item The $N \cdot M$ threads are partitioned into $N$ groups, called {\bf blocks}, of $M$ threads each. \newline
...
\Huge
\begin{center}
{\bf USING blockIdx.x AND threadIdx.x: identify.cu}
\end{center} $\quad$ \newline \LARGE
...
\setkeys{Gin}{width=1.1\textwidth} \includegraphics[scale=0.25,angle=0]{gallant}
...
\Huge
\begin{center}
{\bf ASIDE: AVOIDING PITFALLS IN PARALLELISM}
\end{center} $\quad$ \newline \LARGE

What problem would arise if I used the function, isExectued\_goofup(), instead of Executed(): \newline
\color{red}
\begin{verbatim}
...
\newpage
\color{black}
\setkeys{Gin}{width=1.1\textwidth} \includegraphics[scale=0.25,angle=0]{goofus}
