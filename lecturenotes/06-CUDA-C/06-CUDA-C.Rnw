...
What problem would arise if I changed isExecuted() to: \newline
\color{red}
\begin{verbatim}
...
\begin{itemize}
\item  I run {\tt isExecuted(dev\_a, 0, 0)}
\item Block 0 thread 0 finishes before Block 0 thread 1 finishes.
\end{itemize}

Then, here's what would happen: \newline

\begin{enumerate}[1. ]
\item Block 0 thread 0 would set *dev\_a to 1 because our arguments are blockid = 0 and threadid = 0. 
\item Block 0 thread 1 would set *dev\_a back to 0 because threadid does not equal 1.
\end{enumerate}

So {\tt isExecuted(dev\_a, 1, 1)} might return 0 even it block 1 thread 1 is among the threads that executed.
