...
\end{enumerate} $\quad$ \newline
%JBN: since log_2(n)=m$ why not use m instead of log_2(x)
...
\end{center} $\quad$ \newline
%JBN: again, replace log_2(n) with m
Create $\log_2(n)$ grids, each to compute $X_1, X_2, \ldots, X_{\log_2(n)}$, respectively, in sequence. For the $i$'th grid: \newline
%JBN: this isn't what is actually done. Instead the successive values just overwrite the previous values, e.g. the first thread initially has x_{(0,1)}, then contains x_{(1,1)}, then x_{(2,1)}, then x_{(3,1)}, etc. 
% This means that many of the threads do nothing in steps.
\begin{enumerate}[1. ]
\item Spawn one block of $n/2^i$ threads. %JBN: this will be one block of n threads
...
%JBN: this is all actually done within one kernel call, i.e. one grid
...
%JBN: I would suggest using \cdot instead of the # symbol to make it easier to read
...
%JBN: I'm not sure you are going to have time for this. I typically think about one slide every 2 minutes and you are already at slide 26. And the QR decomposition looks pretty intense. 
\Huge
\begin{center}
...
\begin{itemize}
%JBN: wikipedia has a different definition of QR decomposition. I have no idea if the two are compatible.
\item $Q$ is an $m \times n $ matrix whose columns form an orthonormal basis for the column space of $A$.
\item $R$ is an $n \times n $ upper triangular (and therefore invertible) matrix with all positive entries on the diagonal. %JBN: does it matter that it is invertible?
\end{itemize}
