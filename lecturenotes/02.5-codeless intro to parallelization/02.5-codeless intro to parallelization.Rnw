...
\begin{center}
$\quad$ \newline \newline \newline \newline \newline
{\bf A CODELESS INTRODUCTION TO GPU PARALLELIZATION}
\end{center} $\quad$ \newline
...
{\bf Parallelization}: Running multiple calculations simultaneously. {\color{blue} It speeds up calculations dramatically, and GPUs are much better at it than CPUs.} \newline
...
{\bf REMINDER}
\end{center} $\quad$ \newline \huge
There are several kinds of parallelization, all implemented differently: \newline

\begin{enumerate}[1. ]
\item CPU parallelization
\item GPU parallelization
\item parallel cloud computing
\item parallelization for openMP
\end{enumerate} $\quad$ \newline
