...
\item If $i < m$, set $i = i + 1$ and repeat.  
\end{enumerate}
\newpage
\Huge
\begin{center}
...
\Large
\begin{center}
{\bf PREVIEW OF COMING ATTRACTIONS: A BARE BONES CUDA C WORKFLOW}
\end{center} 
\setkeys{Gin}{width=.8\textwidth} \includegraphics[scale=0.25,angle=0]{picts/skeleton}
