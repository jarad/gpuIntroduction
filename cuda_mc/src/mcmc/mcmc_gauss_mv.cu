...
void FUNC( metropolis_rwpop, TYPE)(int N, int D, float* d_array_init, float sigma,
	...
	switch (D) {
	case 1:
		FUNC( metropolis_rwpop, TYPE) <1> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
		break;
	case 2:
		FUNC( metropolis_rwpop, TYPE) <2> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
		break;
	case 3:
		FUNC( metropolis_rwpop, TYPE) <3> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
		break;
	case 4:
		FUNC( metropolis_rwpop, TYPE) <4> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
		break;
	case 5:
		FUNC( metropolis_rwpop, TYPE) <5> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
		break;
	default:
			break;
	}
...
void FUNC( metropolis_rwpop_marginal, TYPE)(int N, int D, float* d_array_init, float sigma, float* h_args_p,
    ...
    switch (D) {
    case 1:
        FUNC( metropolis_rwpop_marginal, TYPE) <1> (N, d_array_init, sigma, h_args_p, d_temps, d_array_out, log, nb, nt);
        break;
    default:
            break;
    }
